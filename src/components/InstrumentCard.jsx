...
export default function InstrumentCard({
  ...
  const isFavorite =
    propIsFavorite !== undefined ? propIsFavorite : internalIsFavorite;
  ...
  const formatPrice = (price) => {
    ...
    return parseFloat(priceValue).toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
  ...
  const formatChange = (change, percent) => {
    ...
    // Limpiar valores si vienen de AlphaVantage
    const cleanChange =
      typeof change === "string" ? change.replace(/[^0-9.-]/g, "") : change;
    const cleanPercent =
      typeof percent === "string" ? percent.replace("%", "") : percent;

    const isPositive = parseFloat(cleanChange) >= 0;
    const changeSymbol = isPositive ? "+" : "";

    return (
      <span className={`${isPositive ? "text-green-500" : "text-red-500"}`}>
        {changeSymbol}
        {parseFloat(cleanChange).toFixed(2)} (
        {parseFloat(cleanPercent).toFixed(2)}%)
      </span>
    );
  };
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <p className="text-gray-600">
          {instrument.name || "Nombre no disponible"}
        </p>
        <p className="text-lg font-medium">
          {formatPrice(
            instrument.price || instrument.close || instrument["05. price"]
          )}
        </p>
        <p className="text-sm">
          {formatChange(
      ...
      {error && (
        <div className="mt-2 text-sm text-red-500 animate-fade-in">{error}</div>
      )}
    </div>
  );
}
